...
export default function PublicMemorialPage() {
  ...
  const slug = params.slug || "maria-silva-santos";
                  ...
                  {photos.map((photo, index) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                        <p className="absolute bottom-3 left-3 right-3 text-white text-sm truncate">{photo.caption}</p>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                {dedications.map((dedication, index) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <img 
                        src={dedication.authorPhoto || `https://ui-avatars.com/api/?name=${encodeURIComponent(dedication.authorName)}&background=BE123C&color=fff&size=48`}
              ...
              <section className="card-modern p-6 fade-in stagger-3">
                ...
                <div className="space-y-3">
                  {descendants.map((descendant) => (
                    <div key={descendant.id} className="flex items-center gap-3">
                      <img 
                        src={descendant.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(descendant.name)}&background=0F766E&color=fff&size=40`}
                        alt={descendant.name}
                        className="w-10 h-10 rounded-full object-cover"
                      />
                      <div>
                        <p className="font-medium text-gray-900 text-sm">{descendant.name}</p>
                        <p className="text-xs text-gray-500">{descendant.relationship}</p>
                      </div>
                    </div>
                  ))}
                </div>
